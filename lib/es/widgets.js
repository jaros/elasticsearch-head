...
//		path: "",		 		// (required) path to request
	...
	es.ui.ClusterOverview = es.ui.Page.extend({
        ...
        _rebuildIndexAction_handler: function(index) {
            ...
            if(prompt("type 'REBUILD' to rebuild " + index.name + ". There is no undo") === "REBUILD") {
                this.cluster["rebuild"](alias + "/rebuild", null, function(r) {
                    alert(JSON.stringify(r));
                    this.redraw("reset");
                }.bind(this) );
            }
					...
					new acx.ui.MenuButton({
						...
						menu: new acx.ui.MenuPanel({
							items: [
                                ...
                                { text: "Rebuild...", onclick: function() { this._rebuildIndexAction_handler(index); }.bind(this) }
                            ]
						})
					})
				] }
