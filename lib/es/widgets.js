...
//		path: "",		 		// (required) path to request
	...
	es.ui.ClusterOverview = es.ui.Page.extend({
        ...
        _rebuildIndex_handler: function(index, action) {
            var alias = index.metadata.aliases[0];
            if (!alias) {
                alert("Index must have alias to be rebuilt");
                return;
            }
            var answer = prompt("type 'REBUILD' to rebuild " + index.name + ". There is no undo\n" +
                "Type anything to just create duplicate of index.");
            if(!answer) {
                return;
            }
            var rebuild = answer === "REBUILD" ? "substituteIndex" : "duplicateIndex";
            this.cluster["rebuild"](alias + "/" + action + "?command=" + rebuild,  function(r) {
                alert(JSON.stringify(r));
                this.redraw("reset");
            }.bind(this) );
        },
					...
					new acx.ui.MenuButton({
						...
						menu: new acx.ui.MenuPanel({
							items: [
                                ...
                                { text: "Rebuild using local data...", onclick: function() { this._rebuildIndex_handler(index, "localRebuild"); }.bind(this) },
                                { text: "Rebuild totally...", onclick: function() { this._rebuildIndex_handler(index, "totalRebuild"); }.bind(this) }
                            ]
						})
					})
				] }
